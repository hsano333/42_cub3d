...
/*   Updated: 2022/12/29 08:43:26 by hsano            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_point next_map_mass(t_cub3d_type angle, t_cub3d_type x_dist, t_cub3d_type y_dist, t_point map)
{
	...
	if (angle <= M_PI / 2)
	{
		//if (x_dist >= y_dist * fabs(tan(angle)) && tan(angle) != NAN)
		if ((fabs((double)x_dist - fabs(y_dist * tan(angle))) >= F_NEAR) && (tan(angle) != NAN))
	...
	{
		if (x_dist > y_dist * fabs(tan(angle)))
			return (add_map_point(map, 0, -1));
		else
			return (add_map_point(map, 1, 0));
	}
	...
	*/
	...
	else if (angle <= M_PI)
	{
		...
		{
			printf("No.1 x_dist=%lf,calc=%lf ", x_dist, fabs(y_dist *tan(angle)));
			return (add_map_point(map, -1, 0));
		}
		else
		{
			printf("No.2 x_dist=%lf,calc=%lf ", x_dist, fabs(y_dist *tan(angle)));
			return (add_map_point(map, 0, 1));
		}
	}
	else if (angle <= M_PI * 3 / 2)
